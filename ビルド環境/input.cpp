...
void UpdatePad(void)
{
	...
	for (i = 0; i < padCount; i++)
	{
		...
		//* y-axis (forward)
		if (dijs.lRz < 0)					padState[i] |= BUTTON_RUP;
		//* y-axis (backward)
		if (dijs.lRz > 0)					padState[i] |= BUTTON_RDOWN;
		//* x-axis (left)   
		if (dijs.lZ < 0)					padState[i] |= BUTTON_RLEFT;
		//* x-axis (right)
		if (dijs.lZ > 0)					padState[i] |= BUTTON_RRIGHT;
		//* Xボタン
		...
		//* BACKボタン
		if (dijs.rgbButtons[8] & 0x80)		padState[i] |= BUTTON_BACK;
		//* STARTボタン
		 if (dijs.rgbButtons[9] & 0x80)		padState[i] |= BUTTON_START;
		//* 左スティック押し込み
		if (dijs.rgbButtons[10] & 0x80)		padState[i] |= BUTTON_LPRESS;
		//* 右スティック押し込み
		if (dijs.rgbButtons[11] & 0x80)		padState[i] |= BUTTON_RPRESS;
...
#ifdef _DEBUG
	// この文あるとゲームパッド無しだとバグる
	PrintDebugProc("dijs.lY: %d　dijs.lX: %d\n", dijs.lY, dijs.lX);
	PrintDebugProc("dijs.lZ: %d　dijs.lRz: %d\n", dijs.lZ, dijs.lRz);
#endif
}
//----------------------------------------------- 検査
