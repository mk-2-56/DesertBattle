...
#define SHADOWMAP_RESOLUTION 2048		// 解像度
...
static void SetWipeBuffer(void);
static void SetFillBuffer(void);
static void SetMonochromeBuffer(void);
static void SetDissolveBuffer(void);
static void SetShadowBuffer(void);
...
static ID3D11Buffer*				g_FillBuffer = NULL;
...
static FILL_CBUFFER					g_Fill;
static MONOCHROME_CBUFFER			g_Monochrome;
...
// shadowmap材料
static ID3D11SamplerState*			g_ShadowMapSS = NULL;
static ID3D11VertexShader*			g_ShadowMapVS = NULL;
static ID3D11PixelShader*			g_ShadowMapPS = NULL;
static ID3D11InputLayout*			g_ShadowMapIL = NULL;
static ID3D11Texture2D*				g_ShadowMapTex = NULL;
...
{
	g_Fill.FillColor = pFill->FillColor;
	g_Fill.Size = pFill->Size;
...
{
	g_Dissolve.Enable = dissolve->Enable;
	g_Dissolve.ColorType = dissolve->ColorType;
	g_Dissolve.threshold = dissolve->threshold;
...
{
	g_Shadow.Type = shadow->Type;
	g_Shadow.LightViewProj = shadow->LightViewProj;
	g_Shadow.LightViewProj = XMMatrixTranspose(g_Shadow.LightViewProj);
...
HRESULT InitRenderer(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	// ディゾルヴ
	{
		ID3D11ShaderResourceView* g_Fractalnoise;	// テクスチャ情報
		...
		// テクスチャ設定
		GetDeviceContext()->PSSetShaderResources(2, 1, &g_Fractalnoise);
	...
	g_ImmediateContext->VSSetConstantBuffers(8, 1, &g_WipeBuffer);
	g_ImmediateContext->PSSetConstantBuffers(8, 1, &g_WipeBuffer);

	// 塗りつぶし
	hBufferDesc.ByteWidth = sizeof(FILL_CBUFFER);
	g_D3DDevice->CreateBuffer(&hBufferDesc, NULL, &g_FillBuffer);
	g_ImmediateContext->VSSetConstantBuffers(9, 1, &g_FillBuffer);
	g_ImmediateContext->PSSetConstantBuffers(9, 1, &g_FillBuffer);

	// モノクロ
	hBufferDesc.ByteWidth = sizeof(MONOCHROME_CBUFFER);
	g_D3DDevice->CreateBuffer(&hBufferDesc, NULL, &g_MonochromeBuffer);
	g_ImmediateContext->VSSetConstantBuffers(10, 1, &g_MonochromeBuffer);
	g_ImmediateContext->PSSetConstantBuffers(10, 1, &g_MonochromeBuffer);
	...
	// 影
	hBufferDesc.ByteWidth = sizeof(SHADOW_CBUFFER);
	g_D3DDevice->CreateBuffer(&hBufferDesc, NULL, &g_ShadowBuffer);
	g_ImmediateContext->VSSetConstantBuffers(12, 1, &g_ShadowBuffer);
	g_ImmediateContext->PSSetConstantBuffers(12, 1, &g_ShadowBuffer);
	...
	// ここからshadowmapに関するInit作業
	{
		// shadow用シェーダーコンパイル
		{
			...
			hr = D3DX11CompileFromFile("shadowMap.hlsl", NULL, NULL, "VSShadow", "vs_4_0", shFlag, 0, NULL, &pVSBlob, &pErrorBlob, NULL);
			if (FAILED(hr))
			{
				MessageBox(NULL, (char*)pErrorBlob->GetBufferPointer(), "VS", MB_OK | MB_ICONERROR);
			}

			g_D3DDevice->CreateVertexShader(pVSBlob->GetBufferPointer(), pVSBlob->GetBufferSize(), NULL, &g_ShadowMapVS);
			...
			// ピクセルシェーダコンパイル・生成
			ID3DBlob* pPSBlob = NULL;
			hr = D3DX11CompileFromFile("shadowMap.hlsl", NULL, NULL, "PSShadow", "ps_4_0", shFlag, 0, NULL, &pPSBlob, &pErrorBlob, NULL);
			if (FAILED(hr))
			{
				MessageBox(NULL, (char*)pErrorBlob->GetBufferPointer(), "PS", MB_OK | MB_ICONERROR);
			}

			g_D3DDevice->CreatePixelShader(pPSBlob->GetBufferPointer(), pPSBlob->GetBufferSize(), NULL, &g_ShadowMapPS);
		...
		// SRV, DSVの生成
		{
			...
			textureDesc.Width = SHADOWMAP_RESOLUTION;
			textureDesc.Height = SHADOWMAP_RESOLUTION;
			...
			g_D3DDevice->CreateTexture2D(&textureDesc, nullptr, &g_ShadowMapTex);

			D3D11_SHADER_RESOURCE_VIEW_DESC shaderResourceDesc = {};
			shaderResourceDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
			shaderResourceDesc.Texture2D.MipLevels = 1;
			shaderResourceDesc.Format = DXGI_FORMAT_R32_FLOAT;
			g_D3DDevice->CreateShaderResourceView(g_ShadowMapTex, &shaderResourceDesc, &g_ShadowMapSRV);
			...
			g_D3DDevice->CreateDepthStencilView(g_ShadowMapTex, &depthStencilViewDesc, &g_ShadowMapDSV);
		}
		...
		// サンプラーステート設定
		{
			...
			g_D3DDevice->CreateSamplerState(&samplerDesc, &g_ShadowMapSS);

			g_ImmediateContext->PSSetSamplers(1, 1, &g_ShadowMapSS);
		}
	}
...
void UninitRenderer(void)
{
	...
	if (g_FillBuffer)			g_FillBuffer->Release();
	...
	// shadowmap材料
	if (g_ShadowMapSS)			g_ShadowMapSS->Release();
	if (g_ShadowMapVS)			g_ShadowMapVS->Release();
	if (g_ShadowMapPS)			g_ShadowMapPS->Release();
	if (g_ShadowMapIL)			g_ShadowMapIL->Release();
	if (g_ShadowMapTex)			g_ShadowMapTex->Release();
...
{
	// バックバッファクリア
	g_ImmediateContext->ClearDepthStencilView(g_ShadowMapDSV, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);
	// レンダリングターゲットを変更
	g_ImmediateContext->OMSetRenderTargets(0, NULL, g_ShadowMapDSV);

	// シェーダーの変更
	g_ImmediateContext->VSSetShader(g_ShadowMapVS, NULL, 0);
	g_ImmediateContext->PSSetShader(g_ShadowMapPS, NULL, 0);

	// ビューポート設定(解像度の変更などのため)
	D3D11_VIEWPORT vp;
	vp.Width = (FLOAT)SHADOWMAP_RESOLUTION;
	vp.Height = (FLOAT)SHADOWMAP_RESOLUTION;
	vp.MinDepth = 0.0f;
	vp.MaxDepth = 1.0f;
	vp.TopLeftX = 0;
	vp.TopLeftY = 0;
...
void EndShadowSetting(void)
{
	...
	// シェーダーにshadowmapのSRVを渡す
	g_ImmediateContext->PSSetShaderResources(3, 1, &g_ShadowMapSRV);
